...
async def get_me(db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    ...
    return {
        "id": current_user.id, "nombre": current_user.nombre, "apellido": current_user.apellido,
        "username": current_user.username, "email": current_user.email, "cargo": current_user.cargo,
        "empresas": membresias
    }